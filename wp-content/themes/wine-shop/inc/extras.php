 ...
 * @return array
 */
function wineshop_body_classes( $classes ) {
...
add_filter( 'body_class', 'wineshop_body_classes' );
...
if (!function_exists('wineshop_str_replace_assoc')) {

    /**
     * wineshop_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function wineshop_str_replace_assoc(array $replace, $subject) {
 ...
 */
function wineshop_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','wine-shop'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'wineshop_add_to_cart_fragment' );
...
if ( ! function_exists( 'wineshop_breadcrumb_title' ) ) {
	function wineshop_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'wine-shop' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'wine-shop' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'wine-shop' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'wine-shop' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'wine-shop' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'wine-shop' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'wine-shop' ), (get_the_author( '', false ) ));		
...
function wineshop_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','wine-shop'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','wine-shop'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','wine-shop'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
	$before		= '<li class="active">'; // Tag before the current Breadcrumb
	$after 		= '</li>'; // Tag after the current Breadcrumb
	$seprator	= get_theme_mod('seprator','<span style="color: #56032d;margin: 0 10px;">/</span>');
	global $post;
	$homeLink = home_url();
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','wine-shop').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','wine-shop').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','wine-shop').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','wine-shop').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','wine-shop'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','wine-shop') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_wineshop_dismissed_notice_handler', 'wineshop_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function wineshop_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'wineshop_admin_install_plugin' );

function wineshop_admin_install_plugin() {
    /**
     * Install Plugin.
     */
